...
internal class DeterministicRandomizer: IRandomizer
{
    ...
    public void Shuffle<T>(IList<T> list)
    {
        ...
        var guards = casted.Where(c=>c.Rank == CardRank.Guard).Take(3).ToList();
        var priestCard = casted.First(c=>c.Rank == CardRank.Priest);
        var princess= casted.First(c=>c.Rank == CardRank.Princess);
        casted.Clear();
        casted.Add(princess);
